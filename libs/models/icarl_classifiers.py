...
class iCaRLModel(nn.Module):
    ...
    def classify(self, images, method='nearest-mean'):
        if method == 'nme':
            ...
            return self._knn(images)
        elif method == 'svm':
            return self._svm(images)
        elif method == 'bias':
    ...
    def _cosine_similarity(self, images):
        self.eval()
        with torch.no_grad():
            features = self.forward(images).detach()
            ...
            for map in features:
                ...
                dst = {label: (similarity(map, self.exemplars[label]['mean'].to(DEVICE))) for label in
                       self.exemplars.keys()}
